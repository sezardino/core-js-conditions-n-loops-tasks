...
function convertToRomanNumerals(num) {
  ...
  return numerals[1][Math.floor(num / 10)] + numerals[0][num % 10];
}
...
function convertNumberToString(numberStr) {
  ...
  for (; i < numberStr.length; i += 1) {
    ...
    switch (char) {
      ...
      default:
        result += (isNegative && i === 1 ? 'minus ' : '') + digitToWord[char];
        isNegative = false;
        break;
    }
...
function getNearestBigger(number) {
  ...
  const right = digits
    .slice(0, i + 1)
    .concat(digits.slice(i + 1).sort((a, b) => a - b));

  let resultNumber = 0;
  for (let k = right.length - 1; k >= 0; k -= 1) {
    resultNumber = resultNumber * 10 + right[k];
  }
